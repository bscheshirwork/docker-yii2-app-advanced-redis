...
```
phpstorm_helpers:PS-173.3727.138
```
Копируем, например, из запущеного контейнера, средствами `docker` 
```
docker cp phpstorm_helpers_PS-173.3727.138:/opt/.phpstorm_helpers/ .phpstorm_helpers
```
...
```
cd .phpstorm_helpers/
docker build --no-cache --pull -t phpstorm_helpers:PS-173.3727.138 .
```
